...
A full-source audit (structural scans + a live containerised run of the module
with the real dependency stack) found and fixed a cluster of broken export
paths, eleven dead keyboard shortcuts, and two frozen-build risks. See
`mh_PLAYer_v2_12_1_audit_findings.md` for the complete report with reproduction
evidence.

A follow-up colour-path audit then found two silent-corruption bugs in the
`.cube` LUT engine — both fixed below. Neither crashed, errored or warned; both
produced wrong pixels, which is why they survived every prior review pass.

- `VERSION` → `2.12.2`.

### Added
- **Multi-View Compare (View → Multi-View Compare… / new row-2 icon).** A dailies-style side-by-side comparison window: load any supported stills (EXR, PNG, TIFF, JPG, DPX, Cineon, HDR…) via Add Images…, or **Grab Current Frame** to capture the player's canvas exactly as displayed (EV/gamma/display mode/channel baked in — ideal for cross-version frame comparisons). A filmstrip along the bottom holds everything loaded; selecting 1–9 thumbnails fills an auto-layout grid (1×1 → 3×3). Each pane has independent wheel-zoom-about-cursor and drag-pan with a live zoom readout, plus **Sync Zoom & Pan** to mirror inspection across all panes (the comparison feature consumer viewers lack), Fit All and 1:1 All. Window-local keys: ←/→ walk the filmstrip, F fit all, 1 = 1:1 all, Esc closes. One persistent window per session; reuses the existing decode pipeline (`load_exr_raw`/`load_raster_raw`/`raw_to_display`) with EXR shown at a neutral EV 0 / sRGB transform. Engineering note: the window accepts `ShortcutOverride` for its keys so the main window's ApplicationShortcut actions (F, 1, ←, →) don't hijack them while it is active.
- **Help → Check for Latest Version…** Resolves the newest GitHub release tag on a background daemon thread (redirect of `github.com/MHeigan/mh_player/releases/latest` as primary — no API rate limits; API `tag_name` as fallback) and compares it against the running version. Newer release → dialog with a clickable link and an "Open Download Page" button to https://anti-matter-3d.com/mhplayer/; also handles up-to-date, pre-release-build-newer, and offline/error cases. Network work never touches Qt objects; the result returns via a queued signal. Manual check only — nothing runs automatically at startup.
- **Open Sequence dialog: Enter now loads immediately.** "Load All" is the default and focused button, and the dialog always opens preselected at Full (original) resolution — the common case (whole sequence, full res) is a single Enter press. Proxy resolution is now an explicit per-open choice rather than sticky from the app's current proxy state; Load Range / Cancel still fire normally when tabbed to. The default Load All button is highlighted in the same grey as the selected proxy button (new `QPushButton#default_btn` theme rule) so the Enter-default is visually obvious.
- **Canvas zoom accepts manual values.** The transport zoom control is now an editable combo: pick a preset or type any value — `37`, `62.5%`, or `fit` — and press Enter (clamped to the canvas range, 2%–6400%; invalid input silently restores the previous value; focus returns to the player so transport keys work immediately). Presets apply on selection as before. Implementation notes: `currentTextChanged` was replaced by `activated` + `returnPressed` wiring, since with an editable combo it fires on every keystroke and would zoom mid-typing, and a one-shot guard absorbs Qt's double Return dispatch to the combo's line edit.

- **1D shaper support in `.cube` LUTs.** A `.cube` file may legally carry a 1D shaper table and a 3D table together, the shaper running first; mh_PLAYer now reads that combined form (`LUT_1D_SIZE` + `LUT_3D_SIZE` in one file). This is what makes an accurate scene-linear display LUT possible: the shaper distributes the 3D cube nodes logarithmically instead of evenly in linear light. Measured on an ACES viewport LUT, a 65-cube on a plain linear domain misses by 6–20 code values in the shadows; with the shaper it holds neutrals to 0.18 code values and 18% grey to 0.15. `LUT_1D_INPUT_RANGE` and `LUT_3D_INPUT_RANGE` are now parsed as domain aliases alongside `DOMAIN_MIN`/`DOMAIN_MAX` (previously the Iridas spelling was silently dropped and the domain fell back to 0–1). The status line reports what loaded, e.g. `3D 65³ + 1D shaper 4096 · domain [0.00–16.00]`.

### Fixed — LUT engine (.cube) — silent wrong pixels, no error path
- **Every 3D `.cube` LUT was applied with the red and blue axes swapped.** `_load_cube` reshaped the data C-order, which yields `table[B, G, R]` because the format varies red fastest; `_apply_3d` then indexed it as `t[r, g, b]`. The comment directly above the indexing already documented the correct layout — the code below it did the opposite. Demonstrated with the OCIO Sanity Kit's own `identity.cube`, which is a strict identity: pure red `1, 0, 0` came back as pure blue `0, 0, 1`, and `1.0, 0.5, 0.0` as `0.0, 0.5, 1.0`. Neutrals, greys, whites, blacks and any colour where R equals B pass through unaffected, which is exactly why a grey-ramp visual check and several review passes cleared it. Fixed with a transpose at load: `data.reshape(size, size, size, 3).transpose(2, 1, 0, 3)`. Scoped to `_load_cube` — `.3dl` is blue-fastest and already lands as `table[R, G, B]`, so that path is deliberately untouched and is commented as such.
- **Combined shaper + 3D `.cube` files loaded silently corrupted.** The parser collected both tables into one list and took the first `size³` rows, so the shaper rows shifted the 3D table by their own length. An identity-shaper plus identity-3D file — which must be a passthrough — returned `0.5, 0.5, 0.5` → `1.0, 0.0, 0.5`. No warning, no error, just wrong pixels; the format is a normal DaVinci Resolve export, so any customer LUT carrying a shaper was affected. Data rows are now routed to whichever table header was seen most recently and the two tables are kept separate.
- **A `.3dl` loaded after a `.cube` inherited the previous LUT's domain.** `LUTEngine.clear()` never reset `_domain_min` / `_domain_max`, and `_load_3dl()` never set them — only `_load_cube()` assigned a domain. Since `_LUT_ENGINE` and `_SOURCE_LUT_ENGINE` are module-level singletons reused for every load, loading any of the three Latlong Kit LUTs (`DOMAIN_MAX 16.0`) and then a `.3dl` left the stale 0–16 domain in place: the `.3dl` was normalised by 16 and rendered near-black, with the status line still reporting `domain [0.00–16.00]`. Reproduced with a strict-identity `.3dl` (0.5 → 0.031). `clear()` now resets the domain, the shaper ranges and `_is_3d`, and `_load_3dl()` sets the 0–1 domain explicitly — `.3dl` carries no DOMAIN keyword and is always addressed over 0–1.
- **`LUT_1D_INPUT_RANGE` was ignored on 1D-only `.cube` files.** It was parsed, but only consumed on the shaper path, which requires both a 1D *and* a 3D table. A 1D-only LUT declaring `LUT_1D_INPUT_RANGE 0.0 4.0` was therefore normalised over 0–1 and everything above 1.0 clipped to white (input 2.0 returned 1.0 instead of 0.5). `LUT_3D_INPUT_RANGE` already wrote straight to the domain, so the two spellings behaved differently; they are now consistent.
- **`.3dl` output bit depth is inferred rather than assumed.** The parser divided every entry by a hard-coded `4095.0` (12-bit), so a 10-bit `.3dl` read four stops dark and a 16-bit one overflowed ~16× before clipping. The depth is now inferred from the largest value present (10/12/16-bit) and reported in the status line, e.g. `3D 17³, 16-bit`. 12-bit files are byte-for-byte unchanged.
- **A malformed combined LUT is now rejected instead of silently mis-applied.** A file declaring both `LUT_1D_SIZE` and `LUT_3D_SIZE` but carrying too few shaper rows previously dropped the shaper without comment and applied the 3D table on a raw domain — a wrong image rather than an error. It now fails to load, reporting the declared and actual shaper row counts.
- Removed a dead `frac` assignment in `_apply_1d` (computed, never read).
- Corrected two misleading comments in the same area: the axis-order note in `_apply_3d`, and the `_LUT_ENGINE` docstring claiming "display-encoded out" — a display LUT's output is display-**linear**, because `_apply_srgb()` runs on the LUT result afterwards. Authoring against the old wording would have baked the sRGB encode in twice.

### Fixed — export family (all reproduced live before fixing)
- **GUI EXR export (File → Export Frame/Range…) worked in neither mode.** Raw mode: `_write_exr` used `arrays.get(k) or np.zeros(…)` — `or` on a numpy array raises `ValueError` (truth value ambiguous), and the key always exists, so every frame failed. Now an explicit `is None` test with float16 dtype enforcement; round-trip verified byte-exact. Commit-transforms mode: `.exr` was routed through PIL (`unknown file extension`); a new `_write_exr_display()` writes the display-baked frame as a proper float16 EXR — exports exactly what is on screen, per the checkbox tooltip.
- **CLI `--convert -d half` failed on every frame and left corrupt stubs.** The header declared HALF but channels were written as float32 (double the declared size); each failure left a header-only `.exr` stub on disk. Channel bytes now match the declared pixel type, and any write error unlinks the partial file. `-d float` was already correct.
- **16-bit TIFF output was broken in both the CLI (`-d 16`) and the GUI export dialog** — PIL cannot construct an HxWx3 uint16 image (`TypeError` per frame). Both paths now write through `cv2.imwrite` (native 3-channel 16-bit TIFF); verified losslessly round-tripping.
- **CLI `--convert --proxy N` crashed with an unhandled traceback** (`fromarray(…, 'I;16')` on 3-channel input), and both `--proxy` and `--resize` quantised float data mid-pipeline (uint16 / uint8 bounces, HDR clipped to [0,1]). Both now resize in float32 via cv2 (`INTER_AREA` down / `LANCZOS4` up) — HDR preserved through EXR→EXR conversions.

### Fixed — keyboard shortcuts
- **Eleven keys were double-registered** (menu action + `_setup_shortcuts`) and therefore dead: `F` `1` `Home` `End` `Ctrl+E` `Ctrl+I` `Ctrl+Alt+C` `M` `Ctrl+M` `Ctrl+,` `Ctrl+.`. Empirically demonstrated in PySide6 6.11: a key on both a WindowShortcut menu action and an ApplicationShortcut action produces `QAction::event: Ambiguous shortcut overload` and **neither fires** — silently, under the attach console. Each key now has a single owner: the menu action keeps the shortcut (menu hint preserved) promoted to `ApplicationShortcut` (the established I/O/V/[ pattern, so single letters survive spinbox focus), and the `_setup_shortcuts` duplicates are removed. The fixed structure verified firing cleanly in the same harness. This corrects the earlier audit note that claimed the `F`/`1` duplicates "resolve via Qt" — they do not.
- **Sidebar toggle rebound from `Tab` to `Ctrl+Tab`.** Bare Tab is the universal jump-to-next-field key (grading value entry etc.) and kept collapsing the sidebar out of habit; bare Tab now performs normal focus traversal again, and `Ctrl+Tab` keeps the Tab mnemonic. Conflict-free: the app contains no QTabWidget/QTabBar (custom sidebar tabs), so Qt's native Ctrl+Tab tab-cycling cannot be shadowed. Alt+Tab was rejected — OS window switcher, never reaches the app. In-app shortcut help updated.

### Fixed — aspect ratio
- **Aspect ratio persisted globally and silently distorted every newly opened file.** AR was written to `self._prefs['ar_ratio']` on every change and restored at startup, but no open path ever reset it — `_open_file`, `_open_video_file` and `dropEvent` contained no AR handling at all. A single stray selection therefore survived across every subsequent file *and every future session*. Reproduced against the real `_ar_display_size()`: a stale `1:1` (ratio 1.0) is not greater than the source pixel ratio, so the function takes its else-branch and returns `(pw, int(pw / 1.0))` — 1920×1080 renders as **1920×1920** and 1280×720 as **1280×1280**, the reported "video opens at a square ratio". Manually selecting 16:9 returned 1920×1080, identical to Pixel, which is why that workaround appeared to fix it. Note that mh_PLAYer performs no SAR/DAR or resolution-based aspect detection anywhere, so the currently selected AR is applied verbatim to whatever is loaded. AR is now a per-source property: the global prefs write is removed, any stale `ar_ratio` key is purged at startup (without this, an existing install stays squashed after upgrading), and `_open_file()` resets to Pixel next to the existing ROI clear — placed before the video dispatch so it covers video and image sequences alike. New `reset_ar` keyword defaults to `True`; playlist and EDL clip advances pass `reset_ar=False`, since there the AR belongs to the ongoing review (matching the established preserve-EV-across-cut behaviour), and workspace restore is unaffected because it opens the file first and applies its own stored value afterwards. Genuine anamorphic material (non-square pixels) still requires a manual AR selection — SAR-based auto-correction remains a separate feature, not a fix.

### Fixed — frozen-build hardening
- **Rule 21 violations in both CLI FFmpeg paths** (`_cli_encode_video`, `_cli_convert_video_input`): `capture_output=(not verbose)` with no `stdin=DEVNULL` / `CREATE_NO_WINDOW` — the WinError 6 class. Also fixed the compounding verbose-mode crash: on FFmpeg failure `result.stderr` was `None` and `.decode()` raised, masking the real error. Explicit DEVNULL/PIPE + `CREATE_NO_WINDOW` throughout.
- **A PortAudio load failure killed the whole app at import.** The `sounddevice` guard caught only `ImportError`, but a missing/quarantined PortAudio DLL raises `OSError` — reproduced live. Broadened to `except Exception` (matching the PyAV guard): the player now degrades to no-audio instead of failing to launch.

### Fixed — lifecycle, housekeeping & policy
- `closeEvent` now tears down Synced Remote Review: stops the host server (clean shutdown to followers instead of a hard drop), the LAN discovery beacon, and a follower connection — previously none were stopped.
- **`TransportBar._build` assigned `self._r2_compare` twice**, so the attribute referenced the new Multi-View Compare icon rather than the A/B split button created earlier in the same method. Both buttons always worked — the layout and the signal connections hold their own references — but any later `setEnabled()` / `setChecked()` on `self._r2_compare` would silently have hit the wrong icon. The Multi-View button is now `self._r2_multiview`.
- Removed a dead duplicate `def _copy_frame_image` in `PlayerApp` (shadowed by the later definition; a patch trap).
- `PluginHost.app_dir()` no longer relies on `sys.frozen` (Nuitka never sets it; it worked by accident) — frozen detection now matches the module-top bootstrap (executable-name test), correct in source, Nuitka, and PyInstaller-interim modes.
- Removed the banned `adjustSize()` from `_ShortcutDialog.showEvent` (high-DPI collapse class).
- `_open_pdf` gained the standard platform branch (`os.startfile` on Windows, `xdg-open` elsewhere), matching its sibling helpers.
- Module header docstring updated from the stale "v2.0.7" to the current version (historical notes retained).
- RULE D: the CLI burn-in licence message printed the business email to stderr; replaced with the contact-form URL (the in-app mailto remains the sole permitted use of the address).

### Colour QC kits (free downloads, released alongside this version)
- **OCIO / LUT Sanity Kit → kit v1.1.** Files unchanged — they were spec-correct all along; the player was the thing that disagreed with them. README gains a **Channel order** section making `identity.cube` a documented channel-order probe for any tool (load it, feed pure red; a reversed reader returns blue), plus a note in the walkthrough to check the red and blue primary patches rather than the grey ramp, since greys pass either way.
- **ACES Sanity Kit** — unchanged.
- **Display Transform & EV Sanity Kit (new).** A scene-linear EXR chart built so each display mode fails it differently: an exact-stop EV ladder (set EV +1 and a patch must land exactly on its neighbour's old value — a self-check needing no reference numbers), a shadow wedge where sRGB and gamma 2.2 diverge by 5–9 code values, a clipping wedge where ACES keeps climbing past 1.0 while sRGB/Linear/Gamma flatline, and a 0→4.0 gradient. Ships with the expected 8-bit values per mode, computed from the player's own transform code in float32.
- **LatLong / CGI Environment LUT Kit (new).** Three rebuilt LUTs: an ACES viewport preview for the display-LUT slot (65-cube + 4096-point shaper, ACES 1.0 RRT+ODT analytic fit with Bradford-derived sRGB↔AP1 matrices) and, for the source-LUT slot, a luminance-preserving HDRI ambience neutraliser (Bradford CAT, 4500K→D65) and a plate-to-slate match (flare subtract + per-channel gain). The two source LUTs are 17-cube and mathematically exact — a matrix and a per-channel affine are multilinear, and trilinear interpolation reproduces multilinear functions exactly (measured max error 1.1e-05 and 4.8e-06 through the player's own engine). **Requires v2.12.2 or newer:** the viewport LUT hits the combined-file corruption path on earlier builds.
- The originals of these three LUTs were authored against the swapped axis order and are superseded, not migrated.

### Build
- Build bat rev 2: numpy test trees excluded from the compile (`--nofollow-import-to=numpy.conftest` / `numpy.tests` / `numpy._core.tests`) — silences the anti-bloat `doctest`/`setuptools` warnings, speeds compilation, shrinks the dist; the `.pre_versioninfo.bak` injector backup is now deleted automatically after a successful VERSIONINFO injection (kept only when injection fails). Defender resource-lock retries ("error code 22") documented — fix is a one-time Defender exclusion for the build root. `nuitka-build.skill` updated with all three.

### Pre-release audit notes
- The **Display Transform & EV Sanity Kit** README's sRGB column saturated at 254 in the clip-wedge and EV-ladder tables; the player produces 255 for any linear ≥ ~0.9961, since sRGB(1.0) = 1.055 − 0.055 = 1.0 exactly. Seven cells corrected (clip wedge 1.0682/1.1409/1.2136, ladder stops +3…+6), which also resolves the table contradicting its own prose (*"sRGB, Linear and Gamma all hit maximum at 1.0 and stay there"*). All 25 documented rows re-verified against the player's own transform code and its truncating quantiser; the chart EXR/PNG assets are unchanged.
- **`FullscreenPlayer` needs no `ShortcutOverride` handling** — the question raised alongside the CompareWindow work is closed. It is modal (`setModal(True)` + `exec()`), and a modal dialog blocks the parent's ApplicationShortcut actions outright: proven in a PySide6 6.11 harness (modal — 5/5 keys delivered, 0 parent shortcuts fired; modeless without the override — 2 of 3 keys stolen). `CompareWindow` requires the override precisely because it is modeless.

### Known / deferred
- **GUI vs CLI timecode at fractional rates** — the HUD, transport readout, and GUI video-export burn-in still use the float mh_Slate_Tool TC helpers, which diverge from SMPTE at 23.976/29.97/59.94 and never emit drop-frame `;`; the CLI burn-in already uses the exact `EditDecisionList` SMPTE math. Unification is deferred to its own patch and test pass on NTSC material.
